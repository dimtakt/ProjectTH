...
충돌을 관리하는 변수나 enum을 따로 만들어서, 해당 타일의 경우 충돌의 기준이 되는
좌상 우상 좌하 우하의 float 값들을 담는 ColPoint 구조체가 저장되도록 한다
아닌가?

ColPoint 값을 계산하는 건 그냥 함수로 만들면 될 듯 m_tInfo 값 이용하면 되겠지
그냥 Tile 정보를 타일맵에 저장하듯, 충돌 유형도 충돌범위 텍스쳐로 타일맵에 저장하면 될 듯
